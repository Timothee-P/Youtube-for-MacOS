	...
	if (urlPath == "/watch") {
		...
		setTimeout(function() {
			webview.executeJavaScript(
				"if(document.querySelector('.ytp-gradient-bottom.top')){document.querySelector('.ytd-img-top').setAttribute('src',document.querySelector('.ytd-video-owner-renderer img').src);document.querySelector('.ytd-title-top').innerHTML = document.querySelector('h1.ytd-video-primary-info-renderer').innerText;}else{var img = document.createElement('img');img.setAttribute('id','img');img.setAttribute('class','ytd-img-top');img.setAttribute('src',document.querySelector('.ytd-video-owner-renderer img').src);var h1 = document.createElement('h1');h1.setAttribute('class','ytd-title-top');h1.innerHTML = document.querySelector('h1.ytd-video-primary-info-renderer').innerText;var div = document.createElement('div');div.setAttribute('class','ytp-gradient-bottom top');div.appendChild(h1);div.appendChild(img);document.querySelector('#movie_player').appendChild(div);}",
				true
			);
		}, 200);
		setTimeout(function() {
			webview.executeJavaScript(
				"window.ipcTest.send('asynchronous-message-resize',document.getElementsByClassName('video-stream')[0].style.width,document.getElementsByClassName('video-stream')[0].style.height)",
				true
			);
		}, 1500);
		if (oncePreload == false) {
			webview.executeJavaScript(
				"var button1 = document.getElementsByClassName('ytp-miniplayer-button')[0].innerHTML;var button2 = document.getElementsByClassName('ytp-size-button')[0].innerHTML;document.getElementsByClassName('ytp-miniplayer-button')[0].innerHTML=button2;document.getElementsByClassName('ytp-size-button')[0].innerHTML=button1;",
				true
			);
			oncePreload = true;
		}
