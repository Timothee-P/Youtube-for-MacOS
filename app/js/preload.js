		...
		if (once == false) {
			...
			document.getElementsByClassName("ytp-size-button")[0].addEventListener("click", function(e) {
				if (isPIP == false && isPIPpending == false) {
					var height =
						(550 * parseInt(document.getElementsByClassName("video-stream")[0].style.height)) /
						parseInt(document.getElementsByClassName("video-stream")[0].style.width);
	...
	if (document.querySelector("video")) {
		...
		if (document.querySelector("video").hasAttribute("src") && isPIP == false) {
			if (window.scrollY > 0) {
				isScrolling = true;
				if (document.querySelector(".ytp-cards-button")) {
					document.querySelector(".ytp-cards-button").style.transform = "none";
				}
				document.getElementById("masthead-container").style.top = "0";
				ipcRenderer.send("asynchronous-message", "showButton");
			} else {
...
window.addEventListener("wheel", function(e) {
	...
	if (!beginningScrollLeft || !beginningScrollRight) {
		...
		beginningScrollRight =
			document.scrollingElement.scrollWidth - document.scrollingElement.clientWidth - document.scrollingElement.scrollLeft;
	}
